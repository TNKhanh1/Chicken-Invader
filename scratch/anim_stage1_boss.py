...
src_path = "assets/enemy/chicken09.png"
...
# Phoenix boss has wings that flap up and down
...
for f in range(frames):
    ...
    for i, (x, y) in enumerate(src_points):
        ...
        # Body breathing / leg animation
        if y > leg_y_threshold:
            leg_factor = (y - leg_y_threshold) / (h - leg_y_threshold)
            if x < w/2:
                dst_points[i, 0] += math.sin(progress * math.pi * 2) * 5.0 * leg_factor
                dst_points[i, 1] -= math.cos(progress * math.pi * 2) * 3.0 * leg_factor
            else:
                dst_points[i, 0] -= math.sin(progress * math.pi * 2) * 5.0 * leg_factor
                dst_points[i, 1] -= math.cos(progress * math.pi * 2) * 3.0 * leg_factor
    ...
    if np.sum(w_alpha) > 0:
        ...
        cw, ch = cropped.size
        # Kích thước khung nhìn 240x240 để cực kì sắc nét
        scale = min(240 / cw, 240 / ch)
...
out_path = "assets/enemy/chicken09_anim.png"
