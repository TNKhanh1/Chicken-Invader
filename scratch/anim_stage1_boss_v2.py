...
# Remove watermark in bottom-right corner (set alpha to 0)
# Usually watermark is at bottom right 100x100
src_arr[h-80:, w-80:, 3] = 0
...
leg_y_threshold = min_y + actual_height * 0.75

print(f"Calculated Pivots -> Left: {left_pivot:.1f}, Right: {right_pivot:.1f}")
...
for f in range(frames):
    ...
    max_wing_dy = 60.0 * cycle # 60px displacement since image is ~500px tall
    ...
    for i, (x, y) in enumerate(src_points):
        ...
        if y > leg_y_threshold:
            ...
            if x < w/2:
                dst_points[i, 0] += math.sin(progress * math.pi * 2) * 10.0 * leg_factor
                dst_points[i, 1] -= math.cos(progress * math.pi * 2) * 5.0 * leg_factor
            else:
                dst_points[i, 0] -= math.sin(progress * math.pi * 2) * 10.0 * leg_factor
                dst_points[i, 1] -= math.cos(progress * math.pi * 2) * 5.0 * leg_factor
    ...
    if np.sum(w_alpha) > 0:
        ...
        # Add padding to preserve sparks around the wings!
        pad = 20
