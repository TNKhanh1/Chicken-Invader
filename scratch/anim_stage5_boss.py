...
src_path = "assets/enemy/chicken07.png"
...
for f in range(frames):
    ...
    for i, (x, y) in enumerate(src_points):
        ...
        if y > leg_y_threshold:
            ...
            # Legs move inward and outward, and slightly up and down
            # left leg
            if x < w/2:
                dst_points[i, 0] += math.sin(progress * math.pi * 2) * 8.0 * leg_factor
    ...
    if np.sum(w_alpha) > 0:
        ...
        cw, ch = cropped.size
        # Tăng kích thước giới hạn lên 240x240 để sắc nét hơn
...
out_path = "assets/enemy/chicken07_anim.png"
